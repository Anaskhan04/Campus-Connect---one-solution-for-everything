...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (signupForm) {
    signupForm.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        signupSuccess.textContent = `Error: ${error.message}`;
  ...
  if (loginForm) {
    loginForm.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        loginError.textContent = error.message || "Invalid username, password, or role.";
      }
    });
  }
  ...
  // Fixed: Prevent infinite redirect on login page
if (
  !loggedInUser &&
  ...
  // Handle file selection and save image to API
  if (imageInput) {
    imageInput.addEventListener("change", async (event) => {
      ...
      if (file) {
        ...
        try {
          ...
          await api.updateProfileImage(imageDataUrl);
    ...
    const renderTodos = () => {
      ...
      todos.forEach((todo, index) => {
        ...
        li.innerHTML = `<span class="todo-text ${
          ...
          todo.text
        }</span><div class="todo-actions"><button class="complete-btn" data-id="${todo._id}"><i class="fas fa-check"></i></button><button class="delete-btn" data-id="${todo._id}"><i class="fas fa-trash"></i></button></div>`;
    ...
    if (loggedInUser.role === "faculty") {
          ...
          try {
            if (file) {
              ...
              reader.onload = async function (event) {
                noticeData.attachment = event.target.result;
                noticeData.attachmentName = file.name;
      ...
      noticeBoard.addEventListener("click", async (e) => {
        ...
        if (deleteButton) {
          ...
          const noticeId = noticeItem.dataset.id;
          try {
